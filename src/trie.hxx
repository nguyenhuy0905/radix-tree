...
class RadixTree {
  public:
    ...
    /**
     * @brief Remove the specified word from the radix tree, if it is in the
     * tree.
     *
     * @param word
     */
  ...
  private:
    class Node {
      public:
         ...
         *
         */
        auto is_leaf() -> bool { return edge_list.empty(); }
         ...
         * list, if it doesn't already exists. If the string is empty, the key
         * to access the newly inserted edge is 0. Otherwise, it's the first
        ...
        struct Edge {
            ...
            Edge(Node* parent) : label(""), parent(parent) {
                this->child = std::make_unique<Node>(this);
            }
            std::string label;
            /**
             * @brief An unique pointer to the child node of the node containing
             * this edge
             */
            ...
            Node* parent;
            /**
             * @brief Overloads the equivalent operator, so that every time one
             * writes edge1 == edge2, it's comparing each edge's string instead
             * of its entire memory block, which is never equal.
             *
             */
            auto operator==(const Edge& other) -> bool {
                return this->label == other.label;
            }
        };

        /**
         * @brief If a word exists, this contains a no-label edge with key 0.
         * The other words are stored in a pair of form [first character of
         * word, the edge whose label is the word].
         */
         ...
         */
        Edge* incoming_edge;
        explicit Node(Node::Edge* incoming_edge)
     ...
     */
    auto contains_(Node* curr_node, const std::string& suffix) -> bool {
        if (suffix.empty()) {
    ...
    void insert_(Node* curr_node, const std::string& suffix) {
        ...
        // we're there
        if (suffix.empty()) {
            // a leaf node already signals it's the end of word. But a leaf-node
            // root just means there's no word.
        ...
        // We don't need to insert into a leaf due to a leaf already
    ...
    void remove_(Node* curr_node, const std::string& suffix) {
        ...
        if (curr_node->is_leaf()) {
            ...
            if (!suffix.empty())
                return;
            // root node. Simply remove that word, no need to merge with any
            // parent node, since there's none.
            if (curr_node == &root) {
        ...
        std::size_t pos = 0;
        for (; pos < std::min(child_edge_label.length(), suffix.length());
             pos++) {
        ...
        // if we reach here, the word may be there, but we don't know unless we
        // go further down. recursive call
        remove_(curr_node->edge_list[suffix[0]]->child.get(),
                suffix.substr(pos));
    }
};
